...
// 2. 在 T.en 中添加翻译键（在 navReminder 后面）
const enPattern = "navReminder:'Reminder',";
...
if (s.indexOf(enPattern) >= 0 && s.indexOf("navProfile:'Profile'") < 0) {
  s = s.replace(enPattern, enNew);
  console.log('✅ T.en 我的页面翻译已添加');
} else {
  console.log('⚠️ T.en 插入点未找到或已存在');
}

// 3. 在 applyLang 中添加 navProfile
